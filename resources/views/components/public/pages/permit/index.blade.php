    ...
    <section class="">
        <div class="relative w-full bg-neutral-600 py-12 px-4 sm:px-6 md:px-8 lg:px-12 xl:px-16 2xl:px-20">
            ...
            <div class="max-w-[90rem] relative z-10">
                <div class="flex flex-col gap-2">
                    ...
                    <h1 class="text-3xl md:text-4xl font-roboto font-bold text-primary-700 leading-tight">PERIZINAN</h1>
                    <p class="text-white/90 text-sm md:text-base">Pusat informasi dan panduan lengkap terkait proses perizinan di sektor minerba.</p>
                </div>
            </div>
        </div>
        ...
        <div class="relative mx-auto py-12 px-4 sm:px-6 md:px-8 lg:px-12 xl:px-16 2xl:px-20">
            ...
            <div class="mb-6 lg:hidden">
                <label for="permit-category-select" class="block text-sm font-medium text-neutral-700 mb-2">Pilih Kategori</label>
                @if(($categories ?? collect())->isNotEmpty())
                    <select id="permit-category-select" class="w-full rounded-xl border-gray-300 bg-white py-3 px-4 text-sm font-semibold font-roboto shadow-sm focus:outline-none focus:ring-2 focus:ring-primary-700">
                    ...
                    <script>
                        document.addEventListener('DOMContentLoaded', function () {
                            var sel = document.getElementById('permit-category-select');
                            if (sel) {
                                sel.addEventListener('change', function (e) {
                                    var name = e.target.value;
                                    var url = new URL("{{ route('permit.index') }}", window.location.origin);
                ...
                @else
                    <select id="permit-category-select" class="w-full rounded-xl border-gray-300 bg-white py-3 px-4 text-sm font-roboto shadow-sm" disabled>
                ...
                <aside class="md:col-span-3 hidden lg:block">
                    <div class="flex flex-col gap-3">
                        @forelse(($categories ?? collect()) as $c)
                            ...
                            <a href="{{ route('permit.index', ['category' => $c->name]) }}"
                                ...
                                <div class="border-b border-gray-200 pb-6">
                                    <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                                        ...
                                        <div class="flex items-start gap-3 sm:gap-4 flex-1 min-w-0">
                                            ...
                                            <div class="flex-1 min-w-0">
                                                ...
                                                <h3 class="mt-1 sm:mt-2 text-base font-semibold text-gray-900 font-roboto">
                                                    <a href="{{ $doc->download_url ? route('permit.download', $doc->title) : '#' }}" target="_blank" class="hover:text-primary-700 transition-colors duration-300 line-clamp-2">
                                                        {{ $doc->title }}
                                                    </a>
                                                </h3>
                                            </div>
                                        ...
                                        <div class="flex items-center gap-3 self-center max-sm:w-full">
                                            <a href="{{ $doc->download_url ? route('permit.download', $doc->title) : '#' }}"
