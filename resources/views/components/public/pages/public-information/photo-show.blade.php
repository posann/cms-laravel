<x-public.main>
        ...
        <div class="max-w-[90rem] relative z-10">
            <div class="flex flex-col gap-2">
                <div class="mb-4 sm:mb-6 gap-2 sm:gap-4 items-center justify-start flex flex-row flex-wrap text-white font-roboto">
                    ...
                    <span class="text-xs sm:text-sm">&#9679;</span>
                    <span class="text-xs sm:text-sm">Foto</span>
                </div>
    ...
    <div class="m-0 mx-auto w-full max-w-4xl overflow-hidden shadow-sm rounded-2xl relative z-10 -mt-16">
        ...
        @endphp
        <img src="{{ $img }}" alt="{{ $item->title }}" class="w-full h-auto object-cover">
    </div>
    ...
    @if(($related ?? collect())->isNotEmpty())
        <div class="m-0 mx-auto w-full py-6 px-4 sm:px-6 md:px-8 lg:px-12 xl:px-16 2xl:px-20">
            <h2 class="text-xl md:text-2xl font-bold text-gray-900 font-roboto mb-4">Foto Terkait</h2>
                ...
                @foreach($related as $item)
                    <article class="bg-white max-w-sm h-[30rem] rounded-2xl overflow-hidden shadow-sm transition-all duration-300 group hover:bg-primary-700 hover:shadow-lg p-2 flex flex-col">
                        <div class="relative h-[16rem] overflow-hidden rounded-lg">
                            @php
                                $imgRel = $item->image_url ?? asset('images/example/Slider Item 2.png');
                            @endphp
                            <img src="{{ $imgRel }}" alt="{{ $item->title }}"
